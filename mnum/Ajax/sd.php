...
if (decode($_GET["m2"]) == "inda") {
    ...
    $OutTelCode = $YumDamSql->selectsql("SELECT `mytelcode` FROM  $mt2 where `Id`='$di'");
    $OutTelCodeS = $OutTelCode["select"][0]["mytelcode"];


    $IsGoTo = $YumDamSql->updatedb($mt2, "`isstate`='$dt',`mdate`='" . time() . "'", "`mytelcode`='$OutTelCodeS'");
...
if ($IsSelectMySet["row"] == 1 && $YumDamGId != "1" && $YumDamGId != "3") {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        <td align="center" height="40"><a href="javascript:;" onclick="mylistst()" target="_self" class="B">返回手机号码列表</a>
            | <a href="javascript:;"
                 onclick="_GoToLink('Ajax/ld.php?<?= $_SERVER["QUERY_STRING"] . "&is=" . encode("true") . "&d=" . ecode(time()) ?>')"
                 target="_self" class="B">点击查看我独有顾客！</a></td>
    </tr>
</table>
<?
if ($ItemShow["row"] == 0) {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
        ...
        <td><?= MobleNum($ItemShowCont["mytelcode"]) ?></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><?= $ItemShowCont["mybigcity"] . " - " . $ItemShowCont["mycity"] ?></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <?
            if ($ItemShowCont["msgdate"] == "0") {
                echo "未发送短信";
            } else {
                echo "于" . timetodate($ItemShowCont["msgdate"]) . "分发送内容：\n<br>" . $ItemShowCont["msgcont"];
            }
            ?>
        </td>
    </tr>
