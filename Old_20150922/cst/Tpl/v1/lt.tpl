<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    ?>
    <tr
    <?=$iscount==false?"style=\"display:none;\"":"";?>>
    ...
    <td bgcolor="#CCCCCC"><?=round(($totalfr+$itemdata["notnl"]["fr"])/($totaltk-$itemdata["notnl"]["tk"])*100,2);unset($totaltk);unset($totalsy);unset($totalfr);?>
        %
    </td>
    ...
    <tr bgcolor="#999999">
        ...
        <td><?=$lastttfr?></td>
        <td><?=round(($lastttfr+$lastttnlfr)/($lasttttk-$lastttnltk)*100,2);?>%</td>
    </tr>
    <?
    }
    else
    ...
    ?>
    <tr bgcolor="#A3DFFC">
        <td colspan="2" align="center">小计<?=$iscount==false?"(".$tv["ucname"].")":"";?></td>
        <td><?=$ttizwtk;?></td>
        <td><?=$ttizwsy;?></td>
      ...
      ?>
        ...
        <td><?=$attifr?></td>
        <td><?=round(($attifr+$ttinlfr)/($attitk-$ttinltk)*100,2);?>%</td>
