...
$MySql = "$mt2.`myvdate` between " . $date1 . " and " . $date2 . " and $mt2.`isstate` not in(1,6) and (" . $ismy . " `ismyid`='" . $YumDamSId . "') group by `mytelcode` order by $mt2.`isstate` desc, myvdate asc";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    foreach ($ItemList["select"] as $k => $v) {
        $i++;
        if ($YumDamGId == "1" || $YumDamGId == "3") {
            $ismyc = YumDamReadAdmin($v["ismyid"]);
        } else {
        ...
        <tr onmouseout="this.style.backgroundColor='#FFFFFF'" onmouseover="this.style.backgroundColor='#A2E4EE'">
            ...
            <td width="240" align="center"><a href="javascript:;"
                                              ...
                                              target="_self"><?= $v["mytelcode"] ?></a> [<?= $v["tc"] ?>
                ]-<strong>[<?= YumDamMStatus($v["isstate"], $v["mdate"]) ?>]<?= $ismyc . $ismsg ?></strong></td>
            <td>
                <?
                if ($YumDamGId == "1" || $YumDamGId == "3") {
    ...
    <tr>
        <th colspan="7" height="45"><font color="#008000">独</font>：表示该顾客仅有您可以查看；完美处理：有意愿并登记系统或系统有记录；<font
                    color="#DAA520">发</font>：表示已经发送了手机短信！<em>号码后面[数字]</em>：表示当前时间内获取到手机号码次数！
        </th>
    </tr>
</table>
