...
$ListSRes = $YumDamSql->selectsql("SELECT $mt5.`IId`,$mt5.`fdate`,sum($mt5.`ITk`) as `ITk`,sum($mt5.`IIs`) as `IIs`,sum($mt5.`IIf`) as `IIf`,sum($mt5.`IIo`) as `IIo`,sum($mt5.`IIm`) as `IIm`,$mt4.`iname` FROM  $mt5 left join $mt4 on $mt5.`IId`=$mt4.`Id` where $mt5.`fdate` " . YumDamDay($date1, $date2) . " group by $mt5.`IId` WITH ROLLUP");
