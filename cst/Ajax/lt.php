...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    foreach ($DataShowList as $k => $v) {
        if (!empty($v["uid"]) && !empty($v["itemid"])) {
            $i++;
            ?>
            <tr>
                <td><?= $i ?></td>
                <td><?= timetodate($v["fdate"], true) ?></td>
                <td><?= YumDamReadAdmin($v["uid"]) ?></td>
                <td><?= $v["iname"] ?></td>
                <td><?= $v["itemtk"] ?></td>
                <td><?= $v["itemytk"] ?></td>
            ...
            <?
        } elseif (!empty($v["uid"]) && empty($v["itemid"])) {
            ?>
            <tr>
                <td colspan="4">小计</td>
                <td><?= $v["itemtk"] ?></td>
